...
import { FIELD_TYPES, FieldType } from "@/lib/form-schema";
...
export const FieldsSidebar: React.FC<FieldsSidebarProps> = ({ onFieldAdd }) => {
  ...
  const inputFields = Object.entries(FIELD_TYPES).filter(
    ([_, config]) => config.category === "input",
  );
  const udfFields = Object.entries(FIELD_TYPES).filter(
    ([_, config]) => config.category === "udf",
  );
