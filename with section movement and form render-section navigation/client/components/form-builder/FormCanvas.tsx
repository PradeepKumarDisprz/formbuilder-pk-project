...
export const FormCanvas: React.FC<FormCanvasProps> = ({
  ...
  useEffect(() => {
    const handleGlobalDragOver = (e: DragEvent) => {
      ...
      // Check if mouse is near top or bottom of canvas
  ...
  const handleMoveSectionUp = (sectionId: string) => {
    ...
    if (currentIndex > 0) {
      const itemIndex = schema.items.findIndex(
        (item) => isFormSection(item) && item.id === sectionId,
      );
      const prevSection = sections[currentIndex - 1];
      const prevItemIndex = schema.items.findIndex(
        (item) => isFormSection(item) && item.id === prevSection.id,
      );

      handleReorderSection(sectionId, prevItemIndex);
    }
  };
  ...
  const handleMoveSectionDown = (sectionId: string) => {
    ...
    if (currentIndex < sections.length - 1) {
      ...
      const nextItemIndex = schema.items.findIndex(
        (item) => isFormSection(item) && item.id === nextSection.id,
      );

      handleReorderSection(sectionId, nextItemIndex);
    }
  };
  ...
  return (
    <div ref={canvasRef} className="flex-1 bg-gray-100 p-6 overflow-y-auto">
              ...
              <div
                ...
                onDrop={(e) => {
                  ...
                  if (data) {
                    ...
                    handleReorderSection(dragItem.data.sectionId, index);
                  }
                }}
                className="flex items-center gap-3 p-3 bg-white border rounded-lg cursor-grab hover:bg-gray-50"
              >
